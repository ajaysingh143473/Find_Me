...
def find_page(request, item_id):
    ...
    if item.name=="grocery":
        return render(request, 'grow.html', {'item': item,'k':k})

    return render(request, 'findPage.html', {'item': item,'k':k})
def find_page1(request, item_id):
    # Your view logic here...
    item = finepage.objects.get(pk=item_id)
...
def fashion1(request):
    items=finepage.objects.filter(fash='yes')
    return render(request,'homePage.html',{'items':items,'k':k})
def homefurt1(request):
    items=finepage.objects.filter(homefurt='yes')
    return render(request,'homePage.html',{'items':items,'k':k})
def elec1(request):
    items=finepage.objects.filter(elec='yes')
    return render(request,'homePage.html',{'items':items,'k':k})
def book(request):
    items=finepage.objects.filter(boook='yes')
    return render(request,'homePage.html',{'items':items,'k':k})
def grw(request):
    items=finepage.objects.filter(groseries='yes')
    return render(request,'homePage.html',{'items':items,'k':k})
'''def extract_url(url):
